...
/**
 * @brief Alternative calculation of DS1820 temperature sensor value.
 * Temperature in degrees Celcius are calculated from 8-byte temperature-reading data.
 * @note Byte9 is skipped - no CRC is used here.
 */
float calculate_ds18b20_temp_alt(const uint8_t data[8])
...
int main(void)
{
	...
	/* +25 degC */
	tst = calculate_ds18b20_temp(tstData3_A);
	tst_alt = calculate_ds18b20_temp_alt(tstData3_A);
	printf("temp = %.4f and temp_alt = %.4f\n", tst, tst_alt);

	tst = calculate_ds18b20_temp(tstData3_B);
	tst_alt = calculate_ds18b20_temp_alt(tstData3_B);
	printf("temp = %.4f and temp_alt = %.4f\n", tst, tst_alt);
